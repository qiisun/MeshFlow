...
class AutoencoderKL(nn.Module):
    def __init__(self, 
        ...
        if use_encoder:
            self.quant_linear = nn.Linear(hidden_dim, latent_channels) # 9 -> 4*2
    ...
    def encode(self, x, cond, mask):
        ...
        if self.use_encoder:
            ...
            bs, l, _ = x.shape
            moments = x.reshape(bs, l*3, -1)
        mask_expanded = mask.repeat_interleave(3, dim=1) # [bs, 3*N, c]
...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self):
        x = self.mean + self.std * torch.randn_like(self.mean) # consistent noise
...
if __name__ == '__main__':    
    ...
    n, c = 800, 9
    model = AutoencoderKL(latent_channels=9, use_encoder=True).cuda()
